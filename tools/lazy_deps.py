...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # pyproject.toml.
    "camel.tools": ("camel-ai[owl]==0.2.90",),
    # ─── Document → Markdown conversion (plugins/camel_tools/uploads.py) ───
    # Backs the /mnt/user-data/uploads conversion path so PDFs and Office
    # documents become readable text. Depends on markitdown DIRECTLY rather
    # than via camel-ai: conversion should work on installs that never enable
    # the CAMEL bridge, and a converter shouldn't pull CAMEL's dependency tree.
    "doc.markitdown": ("markitdown[all]==0.1.7",),
    # ─── TTS providers ─────────────────────────────────────────────────────
